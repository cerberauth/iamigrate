...
<Aside type="caution">
  Auth0's bulk import requires an email, so Auth0 reports a username-only or phone-only user under `failed` with `TRANSLATION_ERROR`. For Kratos, the identity schema must declare the `username` and `phone` traits as password identifiers.
</Aside>
