...
It also writes `answer-key.json`, pairing each generated user's `source_id` with the cleartext password (and TOTP secret, where enrolled) used to produce their record. This file is gitignored by default and is what lets the live integration test prove a translated hash actually verifies.
...
| `answer-key.json` | Cleartext password + TOTP secret per `source_id` (gitignored) |
